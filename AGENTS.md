...
**当前版本：`1.09`**（2026-08-28 上传的开发版；含：礼盒自选（秋拾/纳福/拾茗，后台已配好）、首页首屏优化（数据预拉取+本地图直出）、分享码进首页修复、后台礼盒管理/余额调整。状态：待设体验版 → 提审发布）。

上传命令（在 `sanmuhe-miniprogram/` 目录执行）：

```bash
VERSION="1.08" DESC="变更摘要" node ../scripts/upload-trial-miniprogram.mjs
```
...
- 版本号**不能重复**使用（微信不覆盖、会新建一条）。已用：`0.1.20260828`、`0.1.20260829`、`0.1.20260830`（可忽略）、`1.08`（含礼盒丢失 bug，勿用）、`1.09`（当前，修复礼盒展示+缓存刷新）。
