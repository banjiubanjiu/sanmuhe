...
**当前版本：`1.10.2`**（2026-08-30 按用户指定版本号重新上传的开发版；含：图片切换不可感知优化、CloudBase WebP 缩略图与预加载、商品详情渐进显示、页面分享体验及 CloudBase 分享封面 URL 修复。状态：待设体验版 → 提审发布）。

上传命令（在 `sanmuhe-miniprogram/` 目录执行）：

```bash
VERSION="1.10.2" DESC="变更摘要" node ../scripts/upload-trial-miniprogram.mjs
```
...
- 通常不要重复使用版本号；若用户明确指定则以用户要求为准。已用：`0.1.20260828`、`0.1.20260829`、`0.1.20260830`（可忽略）、`1.08`（含礼盒丢失 bug，勿用）、`1.09`（含完整礼盒展示、图片、自选茶卡无缩略图和无产地占位修复）、`1.10.2`（当前，已按用户要求重传；含图片切换优化、页面分享体验与云图片封面修复）。
