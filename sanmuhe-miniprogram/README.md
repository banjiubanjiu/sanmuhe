...
未配置真实 AppID/envId 时，当前版本只会回退展示默认商品和内容，不会把下单、预约、报名伪造成真实成功。配置并部署云函数后，茶饮、茶叶、茶室、订单、预约、活动、首页轮播、会员积分、优惠券会优先走云开发。会员充值的真实微信支付目前保持关闭；测试充值产生的余额可验证下单链路，但不能视为真实资金。正式开放前还需完成支付密钥配置、真机小额支付与回调验证，以及余额扣减异常对账方案。订阅消息必须配置与代码字段匹配的微信模板 ID，并由用户授权后才能发送；茶室预约和活动报名仍是提交后人工确认，不做在线收费和自动退款。
