...
- 云开发迁移：已加入云函数根目录和云函数；订单、预约、活动发布/报名、我的记录优先走云函数，未配置云环境时自动回退到本地演示数据；云状态页支持清理自动检查记录。
...
未配置真实 AppID/envId 时，当前版本会自动回退到本地演示数据。配置并部署云函数后，茶饮、茶叶、茶室、订单、预约和活动会优先走云开发。商品支付必须等微信支付商户环境变量、支付回调 HTTP 入口和函数权限配置完成后才能真机实测；茶室预约和活动报名第一版仍是提交后人工确认，不做在线收费和自动退款。
