...
当前版本 **`1.10.2`**；上传用 `VERSION="1.10.2" DESC="..." node ../scripts/upload-trial-miniprogram.mjs`（在 `sanmuhe-miniprogram/` 执行）。该版本已按用户要求上传，包含图片切换不可感知优化、CloudBase WebP 缩略图与预加载、商品详情渐进显示，以及页面分享体验。版本号以用户指定为准，勿自行改用日期号；上传后更新根 AGENTS.md 的「当前版本」段。
